...
HTML_HEADER('取回: '.$row[REPAIR_ARCHIVE_NAME]);
MENU_BAR();
...
VIEW_HEADER('取回');
	...
	VIEW_ROW('送修時間', $row[REPAIR_ARCHIVE_TIME].' ('.RETURN_FRIENDLY_TIME_STR($row[REPAIR_ARCHIVE_TIME]).')');
VIEW_FOOTER();

FORM_HEADER($THIS_PHP_FILE.'&op=archive_save&repair_id='.$repair_id, 'CHECK_EDIT_REPAIR_ARCHIVE');
	FORM_INPUT_TEXT('送修結果', 'repair_report', 64, $row[REPAIR_ARCHIVE_REPORT]);
	FORM_INPUT_TEXT('維修費用', 'repair_cost', 6, $row[REPAIR_ARCHIVE_COST]);
	FORM_DATE_TIME('取回時間', 'repair', $row[REPAIR_ARCHIVE_TIME_RETURN]);
FORM_FOOTER(!empty($THIS_STAFF_RINGS[RING_REPAIR_ARCHIVE_SAVE]), '儲存');
